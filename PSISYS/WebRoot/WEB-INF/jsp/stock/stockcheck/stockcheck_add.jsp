	    ...
	    function calculateTheTotalAmount() {
	        ...
	        if (Str[0] != "") {
	            for (var i = 0; i < Str.length - 1; i++) {
	                ...
	                var pandian = value[6];
	                if(kucun!= ''&& pandian!= ''){
	                	$("#simple-table tr").eq(i+1).children().eq(7).children().eq(0).val(pandian - kucun);
	                }
	            }
	        }
	       // console.log(value);
			...
			$.ajax({
				...
				success: function(data){
					if(data.msg == "success"){//存在商品
						if(data.stockNum >= 0){
							if( GOOD_ID != null)
						    	insertNewRow(GOOD_ID,GOODNAME,BARCODE,UNITNAME,GOODCODE,RPRICE,GOODTYPECODE,GOODSPECIF,data.stockNum);
